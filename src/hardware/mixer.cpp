...
static std::optional<AudioFrame> parse_volume(const std::string& s)
{
	...
	if (parts.size() == 1) {
		// Single volume value
		if (const auto value = to_volume(parts[0]); value) {
			return AudioFrame(*value, *value);
		}
	} else if (parts.size() == 2) {
		// Stereo volume value
		const auto left  = to_volume(parts[0]);
		const auto right = to_volume(parts[1]);
		if (left && right) {
	...
	void Run() override
	{
		...
		for (auto& arg : args) {
			...
			if (global_command) {
				...
				} else if (p = parse_prefixed_percentage(reverb_command,
				                                         arg);
				           p) {
				...
				} else if (p = parse_prefixed_percentage(chorus_command,
				                                         arg);
				           p) {
					const auto level = percentage_to_gain(*p);
